...
public:
	...
	virtual void OnPaint()
	{
		...
		if (this->sel_index != -1 && !HasBit(buttons, this->sel_index)) this->sel_index = -1;
	...
	virtual void UpdateWidgetSize(int widget, Dimension *size, const Dimension &padding, Dimension *fill, Dimension *resize)
	{
		switch (widget) {
			...
			case WID_TA_COMMAND_LIST:
				...
				for (uint i = 0; i < TACT_COUNT; i++ ) size->width = max(size->width, GetStringBoundingBox(STR_LOCAL_AUTHORITY_ACTION_SMALL_ADVERTISING_CAMPAIGN + i).width);
				size->width += WD_FRAMERECT_LEFT + WD_FRAMERECT_RIGHT;
				break;
	...
	virtual void DrawWidget(const Rect &r, int widget) const
	{
		...
		for (int i = TE_BEGIN; i < TE_END; i++) {
			...
			if (this->town->goal[i] == TOWN_GROWTH_DESERT || this->town->goal[i] == TOWN_GROWTH_WINTER) {
				...
				if (this->town->received[i].old_act < this->town->goal[i]) string = STR_TOWN_VIEW_CARGO_FOR_TOWNGROWTH_REQUIRED;
	...
	virtual void OnClick(Point pt, int widget, int click_count)
	{
		switch (widget) {
			case WID_TV_CENTER_VIEW: // scroll to location
				if ((_ctrl_pressed || _ctrl_toolbar_pressed)) ShowExtraViewPortWindow(this->town->xy);
				else ScrollMainWindowToTile(this->town->xy);
				break;
...
{
	if (_game_mode == GM_EDITOR) AllocateWindowDescFront<TownViewWindow>(&_town_editor_view_desc, town);
	else AllocateWindowDescFront<TownViewWindow>(&_town_game_view_desc, town);
}
...
public:
	...
	virtual void UpdateWidgetSize(int widget, Dimension *size, const Dimension &padding, Dimension *fill, Dimension *resize)
	{
		switch (widget) {
			...
			case WID_TD_SORT_CRITERIA: {
				Dimension d = {0, 0};
				for (uint i = 0; TownDirectoryWindow::sorter_names[i] != INVALID_STRING_ID; i++) d = maxdim(d, GetStringBoundingBox(TownDirectoryWindow::sorter_names[i]));
...
public:
	...
	void UpdateButtons(bool check_availability)
	{
		...
		for (int i = WID_TF_SIZE_SMALL; i <= WID_TF_SIZE_RANDOM; i++) this->SetWidgetLoweredState(i, i == WID_TF_SIZE_SMALL + this->town_size);
		this->SetWidgetLoweredState(WID_TF_CITY, this->city);
		for (int i = WID_TF_LAYOUT_ORIGINAL; i <= WID_TF_LAYOUT_RANDOM; i++) this->SetWidgetLoweredState(i, i == WID_TF_LAYOUT_ORIGINAL + this->town_layout);
		this->SetDirty();
	}
	...
	virtual void OnClick(Point pt, int widget, int click_count)
	{
		switch (widget) {
			...
			case WID_TF_MANY_RANDOM_TOWNS:
				...
				if (!GenerateTowns(this->town_layout)) ShowErrorMessage(STR_ERROR_CAN_T_GENERATE_TOWN, STR_ERROR_NO_SPACE_FOR_TOWN, WL_INFO);
...
public:
	...
	int FindHouseSet(HouseID house) const
	{
		if (house != INVALID_HOUSE_ID) {
			...
			for (uint i = 0; i < this->NumHouseSets(); i++) if (HouseSpec::Get(this->GetHouseAtOffset(i, 0))->grf_prop.grffile == house_set) return i;
		}
		return -1;
	}
	...
	*/
	...
	void Build()
	{
		...
		for (HouseID house = 0; house < NUM_HOUSES; house++) if (IsHouseTypeAllowed(house, zones, _game_mode == GM_EDITOR).Succeeded()) *this->Append() = house;
		this->Compact();
...
protected:
	...
	{
		if (_settings_game.economy.allow_all_houses || _game_mode == GM_EDITOR) return false;
	...
	void SelectOtherHouse(int new_house_set, int new_house_offset, int new_house_variant, int clicked)
	{
		...
		if (clicked > 0 ? disabled : this->IsObjectToPlaceSet()) ResetObjectToPlace(); // warning, this may cause recursion through OnPlaceObjectAbort
		...
		if (clicked > 0 && !disabled) {
			...
			SetTileSelectSize(hs->building_flags & BUILDING_2_TILES_X ? 2 : 1, hs->building_flags & BUILDING_2_TILES_Y ? 2 : 1);
		}
	}
	...
	{
		uint w = 1, h = 1;
		if (_cur_house.id != INVALID_HOUSE_ID) {
	...
	virtual void UpdateWidgetSize(int widget, Dimension *size, const Dimension &padding, Dimension *fill, Dimension *resize)
	{
		switch (widget) {
			case WID_HP_HOUSE_SETS: {
				uint max_w = 0;
				for (uint i = 0; i < this->house_list.NumHouseSets(); i++) max_w = max(max_w, GetStringBoundingBox(this->house_list.GetNameOfHouseSet(i)).width);
	...
	virtual void DrawWidget(const Rect &r, int widget) const
	{
		switch (GB(widget, 0, 16)) {
			...
			case WID_HP_HOUSE_SELECT: {
				...
				DrawHouseImage(house, r.left + WD_IMGBTN_LEFT + lowered, r.top + WD_IMGBTN_TOP + lowered, r.right - WD_IMGBTN_RIGHT + lowered, r.bottom - WD_IMGBTN_BOTTOM + lowered, HIT_GUI_HOUSE_LIST);
				/* grey out outdated houses */
				if (!this->IsHouseDisabled(house)) break;
				GfxFillRect(r.left + WD_BEVEL_LEFT, r.top + WD_BEVEL_TOP, r.right - WD_BEVEL_LEFT, r.bottom - WD_BEVEL_BOTTOM, PC_BLACK, FILLRECT_CHECKER);
				break;
			}
		}
	}
	...
	virtual void OnClick(Point pt, int widget, int click_count)
	{
		switch (GB(widget, 0, 16)) {
			case WID_HP_HOUSE_SETS: {
				...
				if (index < this->house_list.NumHouseSets()) this->SelectOtherHouse(index, -1, -1, -1);
				break;
			}
...
struct SelectTownWindow : Window {
	...
	StringID GetTownString(uint i) const
	{
		...
		if (CheckHouseDistanceFromTown(Town::Get(this->towns[i]), this->cmd.tile, false).Failed()) return STR_SELECT_TOWN_LIST_TOWN_OUTSIDE;
	...
	virtual void UpdateWidgetSize(int widget, Dimension *size, const Dimension &padding, Dimension *fill, Dimension *resize)
	{
		...
		Dimension d = { 0, 0 };
		for (uint i = 0; i < this->towns.Length(); i++) d = maxdim(d, GetStringBoundingBox(this->GetTownString(i)));
...
struct TownsToJoinHouseToListSorter {
	...
	bool operator () (const TownID &a, const TownID &b) const
	{
		...
		return (uint64)(dist_a - inner_a) * (uint64)(outer_b - inner_b) < (uint64)(dist_b - inner_b) * (uint64)(outer_a - inner_a);
	}
};
...
static CommandCost ListTownsToJoinHouseTo(HouseID house, TileIndex tile, TownList *towns)
{
	...
	FOR_ALL_TOWNS(t) {
		...
		if (!_settings_game.economy.allow_all_houses && (!deity && !HasBit(HouseSpec::Get(house)->building_availability, GetTownRadiusGroup(t, tile)))) {
